...
TRAEFIK_ACME_DOMAINS=${TRAEFIK_ACME_DOMAINS:="false"}
TRAEFIK_ACME_MAIN_DOMAIN=${TRAEFIK_ACME_MAIN_DOMAIN:-""}
TRAEFIK_ACME_SANS_DOMAIN=${TRAEFIK_ACME_SANS_DOMAIN:-""}
...
"
    ...
    if [ "${TRAEFIK_ACME_DOMAINS}" == "true" ]; then
        TRAEFIK_ACME_CFG=${TRAEFIK_ACME_CFG}"\
  [acme.domains]
    main = \"${TRAEFIK_ACME_MAIN_DOMAIN}\"
    sans = [\"${TRAEFIK_ACME_SANS_DOMAIN}\"]
"
    fi
